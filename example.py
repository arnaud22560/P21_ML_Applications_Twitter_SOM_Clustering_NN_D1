...
try:
    ...
    table = Datasheet.load(pd("Music.csv"))
...
for i in range(2):
    print(i)
    for tweet in engine.search("music", start=prev, count=1000, cached=False):
        print("")
        print(tweet.text)
        print(tweet.author)
        print(tweet.date)
